      ...
      <section className="auth-frame">
        <aside className="auth-aside">
          <div className="auth-brand">
            <img src={graduationCapIcon} alt={translate("학사모 아이콘")} className="brand-icon" />
            <b>STUDYMATE</b>
          </div>
          <div>
            <h1>{translate("나의 완벽한")}<br />{translate("스터디 파트너를")}<br />{translate("찾아보세요.")}</h1>
            <p>{translate("전 세계 유학생들과 함께 전공 지식을 나누고,")}<br />{translate("언어를 교환하며 함께 성장하는 지적인 커뮤니티입니다.")}</p>
          </div>
          <div className="live-card">
            <AvatarStack />
            <strong>{translate("지금 1,200명의 학우들이 함께")}<br />{translate("공부하고 있습니다.")}</strong>
          </div>
        </aside>
        <section className="auth-panel">
          <header className="auth-header">
            ...
            <div className="lang-selector">
              ...
              >
                ...
                {lang.label} ({lang.shortLabel}) <i className="arrow-down" />
              </button>
          ...
          <div className="auth-content">
            ...
            <div className={`auth-phase ${isLogin ? 'active' : 'inactive-left'}`}>
              <h2>{translate("반가워요, 학우님!")}</h2>
              <p>{translate("서비스 이용을 위해 학교 계정으로 로그인 해주세요.")}</p>
              <form className="form-stack" onSubmit={(event) => {
                ...
                {message && isLogin && <p className={`auth-feedback login-feedback ${message.type}`}>{translate(message.text)}</p>}
                <button className="primary wide" type="submit" disabled={isSubmitting}>
                  {isSubmitting ? translate("로그인 중...") : translate("로그인")} <span>→</span>
                </button>
              </form>
              <footer className="auth-footer">
                <p>{translate("계정이 없으신가요?")} <button type="button" onClick={() => {
                  navigate(ROUTE_PATHS.signup);
                  resetAuthState();
                }}>{translate("지금 가입하세요")}</button></p>
              </footer>
            </div>

            {/* === 회원가입: 스텝 1 === */}
            <div className={`auth-phase ${!isLogin && signupStep === 1 ? 'active' : isLogin ? 'inactive-right' : 'inactive-left'}`}>
              <h2>{translate("새로운 시작을 함께해요!")}</h2>
              <p>{translate("스터디 메이트가 되기 위해 이메일 인증을 진행해 주세요.")}</p>
              <form className="form-stack" onSubmit={(event) => {
                event.preventDefault();
                handleVerifyCode();
              }}>
                ...
                <button className="verify-btn" type="button" onClick={handleSendCode} disabled={isSendingCode || isVerifyingCode}>
                  {isSendingCode ? translate("전송 중...") : translate("인증코드 보내기")}
                </button>
                ...
                {message && !isLogin && signupStep === 1 && <p className={`auth-feedback signup-feedback ${message.type}`}>{translate(message.text)}</p>}
                <button className="primary wide" type="submit" disabled={isVerifyingCode}>
                  {isVerifyingCode ? translate("확인 중...") : translate("다음 단계")} <span>→</span>
                </button>
              </form>
            </div>
            ...
            <div className={`auth-phase ${!isLogin && signupStep === 2 ? 'active' : 'inactive-right'}`}>
              <h2>{translate("거의 다 왔어요!")}</h2>
              <p>{translate("비밀번호와 관심사를 설정해 주세요.")}</p>
              <form className="form-stack" onSubmit={(event) => {
                event.preventDefault();
                ...
                {message && !isLogin && signupStep === 2 && <p className={`auth-feedback signup-feedback ${message.type}`}>{translate(message.text)}</p>}
            ...
            <div className={`auth-phase ${!isLogin && signupStep === 3 ? 'active' : 'inactive-right'}`}>
              <h2>{translate("프로필을 완성해요")}</h2>
              <p>{translate("스터디 매칭에 사용할 기본 정보를 입력해 주세요.")}</p>
              <form className="form-stack" onSubmit={(event) => {
                event.preventDefault();
                handleSignup();
              }}>
                ...
                <label>
                  {translate("자기소개")}
                  <textarea
                    placeholder={translate("관심 분야와 함께 공부하고 싶은 목표를 적어주세요.")}
                    value={bio}
                    onChange={(event) => setBio(event.target.value)}
                    disabled={isSubmitting}
                  />
                </label>
                {message && !isLogin && signupStep === 3 && <p className={`auth-feedback signup-feedback ${message.type}`}>{translate(message.text)}</p>}
                ...
                <div className="button-group">
                  ...
                  <button className="primary wide" type="submit" disabled={isSubmitting}>
                    {isSubmitting ? translate("가입 중...") : translate("회원가입 완료")} <span>→</span>
                  </button>
                </div>
              </form>
            </div>
          ...
          <nav className="global-legal-nav">
            <span>{translate("이용약관")}</span>
            <span>{translate("개인정보처리방침")}</span>
            <span>{translate("도움말")}</span>
          </nav>
        </section>
      </section>
    </main>
